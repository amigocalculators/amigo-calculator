...
export default function EventPageClient({ exhibitions, galleryImages }: Props) {
  ...
  const groupedExhibitions = exhibitions.reduce<Record<number, Exhibition[]>>((acc, exhibition) => {
    ...
    <div className="min-h-screen bg-[#f0efef]">
      ...
      <main className="container mx-auto px-4 py-8 mt-16">
        ...
        <section id="past" className="pt-12">
          ...
          {Object.entries(groupedExhibitions)
            .sort(([a], [b]) => Number(b) - Number(a))
            .map(([year, exList]) => (
              <div key={year} className="mb-16">
                <div className="flex items-center gap-4 mb-8">
                  <h3 className="text-2xl font-bold text-rose-600">{year}</h3>
                  <div className="grow h-px bg-rose-200" />
                </div>
            ...
            {galleryImages.map((image, index) => (
              ...
              >
                <img
                  ...
                  className="w-full h-75 object-cover transition-transform group-hover:scale-110 duration-500"
                />
              </div>
            ))}
          </div>
        ...
        >
          <div className="bg-white rounded-xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            <div className="relative">
              ...
              <img src={selectedExhibition.image} alt={selectedExhibition.name} className="w-full h-75 md:h-125 object-cover" />
            </div>
            <div className="p-6 md:p-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8 text-slate-600">
                <div className="flex items-center"><MapPin size={20} className="mr-3 text-rose-600 shrink-0" /><span>{selectedExhibition.location}</span></div>
                <div className="flex items-center"><Calendar size={20} className="mr-3 text-rose-600 shrink-0" /><span>{selectedExhibition.dates}</span></div>
                <div className="flex items-center"><Building2 size={20} className="mr-3 text-rose-600 shrink-0" /><span>{selectedExhibition.venue}</span></div>
              </div>
            </div>
          </div>
        </div>
